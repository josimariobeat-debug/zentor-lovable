/*! Zentor Widget viewer v3 — player, produtos, comentários, curtidas, share.
 *  Carregado sob demanda pelo core.js no 1º hover/click em miniatura.
 *  Expõe window.__ZENTOR_VIEWER__.open({ stories, startIdx, shadow, track, storeId }).
 */
(function () {
  'use strict';
  ...
  var VIEWER_STYLES = [
    ...
    '.zt-product-btn{background:#111;color:#fff;border:0;border-radius:0 0 7px 7px;padding:7px 14px;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;flex-shrink:0;transition:background .15s}',
  ...
  function ensureStyles(shadow) {
    if (shadow.__ztViewerStyled) return;
    shadow.__ztViewerStyled = true;
    var st = document.createElement('style');
    st.textContent = VIEWER_STYLES;
  ...
  function openPlayer(opts) {
    ...
    function blockNativeMediaAction(e) { try { e.preventDefault(); e.stopPropagation(); } catch(_){} return false; }
    ...
    function nextOnce(reason) { if (advancing) return; advancing = true; next(); }
    ...
    function show() {
      ...
      if (isVideo) {
        ...
        function tick(now) {
          ...
          } catch (err) { rafId = requestAnimationFrame(tick); }
        }
        ...
        function tick2(t) {
          ...
          } catch (err) { rafId = requestAnimationFrame(tick2); }
        }
        rafId = requestAnimationFrame(tick2);
      }
      track('view', story.id);
  ...
  window.__ZENTOR_VIEWER__ = { open: openPlayer, version: '3' };
})();
